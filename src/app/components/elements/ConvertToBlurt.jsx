...
class ConvertToBlurt extends React.Component {
    constructor() {
        super();
        this.state = {};
    }
    ...
    shouldComponentUpdate = shouldComponentUpdate(this, 'ConvertToBlurt');
    dispatchSubmit = () => {
            ...
            >
                <div className="row">
                    <div className="small-12 columns">
                        <h1>
                            {tt('converttoblurt_jsx.convert_to_LIQUID_TOKEN', {
                                LIQUID_TOKEN,
                            })}
                        </h1>
                        <p>
                            {tt(
                                'converttoblurt_jsx.DEBT_TOKEN_will_be_unavailable',
                                { DEBT_TOKEN }
                            )}
                        </p>
                        <p>
                            {tt(
                                'converttoblurt_jsx.your_existing_DEBT_TOKEN_are_liquid_and_transferable',
                                { link: tt('g.buy_or_sell'), DEBT_TOKEN }
                            )}
                        </p>
                        <p>
                            {tt(
                                'converttoblurt_jsx.this_is_a_price_feed_conversion'
                            )}
                        </p>
                    </div>
                </div>
...
export default reduxForm(
    { form: 'convertToBlurt', fields: ['amount'] },
    // mapStateToProps
    (state, ownProps) => {
                    ...
                    successCallback: () => {
                        ...
                        dispatch(
                            appActions.addNotification({
                                key: 'convert_sd_to_blurt_' + Date.now(),
