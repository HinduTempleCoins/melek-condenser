...
async function blurtGetCanvasHash() {
	try {
		...
		ctx.fillText('MELEK account creator fingerprint', 8, 8);
...
function downloadKeysFile(username, keys, master) {
  ...
  a.href = url;
  a.download = username + "_melek_keys_backup.txt";
...
function pdfSafeFilename(value) {
	return String(value || 'melek-account')
		.toLowerCase()
		.replace(/[^a-z0-9._-]/g, '-')
		.replace(/-+/g, '-')
		.replace(/^-|-$/g, '') || 'melek-account';
}
...
function createPdfDocument(pages, images) {
	...
	let pdf = '%PDF-1.4\n% generated by MELEK account creator\n';
	const offsets = [0];
...
function addPdfHeader(page, title, username, logoImage) {
	...
	page.text('IMPORTANT: This PDF contains private MELEK keys. Keep it secret and offline.', 40, 84, 9);
...
async function downloadKeysPackage(username, keys, master) {
	try {
		...
		addPdfHeader(page, 'MELEK Account Backup', username, logoImage);
		...
		forceDownloadFile(blob, safeUsername + '_melek_account_backup.pdf');

		setTimeout(() => {
			alert('Your MELEK backup PDF has been generated. Save this file now. Without it you may lose access to your account.');
		}, 250);
...
function showSuccessMessage(username, master, posting) {
  ...
  container.innerHTML = `
    ...
    To log in to MELEK, download the <strong>WhaleVault</strong> browser extension and use your <strong>Master Password</strong> to setup all keys.<br><br>
