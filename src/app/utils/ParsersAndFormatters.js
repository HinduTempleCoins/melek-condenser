...
export function formatLargeNumber(number, decimals) {
    if (number < 1e3) return `${number.toFixed(decimals)}`;
    if (number < 1e6) return `${(number / 1e3).toFixed(decimals)} K `;
    if (number < 1e9) return `${(number / 1e6).toFixed(decimals)} M`;
    if (number < 1e12) return `${(number / 1e9).toFixed(decimals)} B `;
}
