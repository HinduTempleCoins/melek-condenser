 ...
 *    - basic sanitization?
 *  2. Blurt.blog Rendering - add in proprietary Blurt.blog functions/links
...
function link(state, child) {
    ...
    if (url) {
        state.links.add(url);
        if (state.mutate) {
            // If this link is not relative, http, https, or blurt -- add https.
...
function ipfsPrefix(url) {
    if ($STM_Config.ipfs_prefix) {
        // Convert //ipfs/xxx  or /ipfs/xxx  into  https://blurt.blog/ipfs/xxxxx
        if (/^\/?\/ipfs\//.test(url)) {
